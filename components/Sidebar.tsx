...
function SidebarLink({
  ...
  return (
    <Link
      ...
      className={
        active
          ? `${baseLinkClass} border-indigo-500/20 bg-indigo-500/10 text-indigo-300 shadow-[0_0_20px_rgba(99,102,241,0.08)]`
          : `${baseLinkClass} border-transparent text-slate-400 hover:border-slate-800 hover:bg-slate-900/80 hover:text-slate-100`
      }
    >
      <Icon
        size={19}
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-50 flex h-screen w-64 flex-col border-r border-slate-800 bg-slate-950 px-4 py-6">
      <div className="mb-10 shrink-0 px-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-600/20 text-xl">
            🇰🇷
          </div>
          <div>
            <h1 className="text-sm font-bold tracking-wide text-white">K-ROADMAP</h1>
            <p className="text-xs text-slate-500">Korea 2027</p>
          </div>
        </div>
      </div>
      ...
      <nav className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
        ...
        <div className="pt-1">
          ...
          >
            <GraduationCap
              size={19}
              className={isMajorsSection ? 'text-indigo-400' : 'text-slate-500'}
            />
            <span className="flex-1">Majors</span>
            <ChevronDown
              size={16}
              className={`transition-transform ${isMajorsSection ? 'rotate-180 text-indigo-400' : 'text-slate-600'}`}
            />
          </Link>

          {isMajorsSection && (
            <div className="ml-5 mt-1 space-y-1 border-l border-slate-800 pl-3">
              {majorNavigation.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={
                      active
                        ? 'flex items-center gap-2 rounded-lg bg-indigo-500/10 px-3 py-2.5 text-xs font-semibold text-indigo-300'
                        : 'flex items-center gap-2 rounded-lg px-3 py-2.5 text-xs font-medium text-slate-500 transition-colors hover:bg-slate-900 hover:text-slate-200'
                    }
                  >
                    <Icon size={15} />
                    <span>{item.name}</span>
      ...
      <div className="mt-4 shrink-0 rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-xs font-semibold text-slate-400">JOURNEY STATUS</span>
          <span className="h-2 w-2 rounded-full bg-emerald-400" />
        </div>
        <p className="text-xs leading-relaxed text-slate-500">
          Building the path, one step at a time.
        </p>
      </div>
    </aside>
  );
