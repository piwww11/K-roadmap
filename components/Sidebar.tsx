  ...
  return (
    <aside className="fixed left-0 top-0 z-50 flex h-screen w-64 flex-col border-r border-slate-800 bg-slate-950 px-4 py-6">
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={isActive ? 'group flex items-center gap-3 rounded-xl border border-indigo-500/20 bg-indigo-500/10 px-3 py-3 text-sm font-medium text-indigo-300 shadow-[0_0_20px_rgba(99,102,241,0.08)] transition-all duration-200' : 'group flex items-center gap-3 rounded-xl border border-transparent px-3 py-3 text-sm font-medium text-slate-400 transition-all duration-200 hover:border-slate-800 hover:bg-slate-900/80 hover:text-slate-100'}>
              <Icon size={19} className={isActive ? 'text-indigo-400' : 'text-slate-500 transition-colors duration-200 group-hover:text-slate-300'} />
      ...
      <div className="mt-auto rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        <div className="mb-2 flex items-center justify-between"><span className="text-xs font-semibold text-slate-400">JOURNEY STATUS</span><span className="h-2 w-2 rounded-full bg-emerald-400" /></div>
        <p className="text-xs leading-relaxed text-slate-500">Building the path, one step at a time.</p>
      </div>
    </aside>
  );
}
