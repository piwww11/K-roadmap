...
-- Schema-only: intentionally not wired into the application yet.
-- Existing localStorage is untouched until a later migration phase.
-- Intelligence outputs remain derived and are not stored.
-- payload JSONB preserves client-only/nested fields during first migration.
...
begin
  foreach table_name in array array[
    'profiles','journey_phases','journey_months','journey_goals','journey_tasks',
    'majors','skills','major_decisions','journal_entries','documents','achievements',
    'experiments','experiment_attempts','experiment_reflections','applications',
    'budget_profiles','budget_items','saving_transactions'
  ] loop
...
begin
  foreach table_name in array array[
    ...
    'majors','skills','major_decisions','journal_entries','documents','achievements',
    'experiments','experiment_attempts','experiment_reflections','applications',
    'application_documents','budget_profiles','budget_items','saving_transactions'
  ] loop
...
select
  ...
  case when bp.target_amount > 0
    then round((coalesce(sum(st.amount), 0) / bp.target_amount) * 100, 1)
    else 0
