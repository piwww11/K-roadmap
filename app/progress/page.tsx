...
import { BarChart3, CalendarDays, CheckCircle2, Clock3, FlaskConical, ArrowRight, BookOpen, Brain, ChevronDown, ChevronUp } from 'lucide-react';
import { useMemo, useState } from 'react';
import { useJourneyStore } from '@/store/useJourneyStore';
import { useExperimentStore } from '@/store/experimentStore';
import { buildProgressAnalytics, dateLabel } from '@/lib/progressAnalytics';

const eventIcon = { decision: Brain, experiment: FlaskConical, reflection: CheckCircle2, journal: BookOpen };
    ...
    <main className="min-h-screen bg-slate-950 p-8 text-slate-100">
      <div className="mx-auto max-w-6xl">
        <header className="mb-10">
          ...
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><h1 className="text-4xl font-bold text-white">Progress Analytics</h1><p className="mt-3 max-w-3xl text-slate-400">See how much of the journey is complete, where progress is concentrated, and which dated activities have shaped the journey so far.</p></div><Link href="/roadmap" className="inline-flex items-center gap-2 rounded-xl border border-slate-700 px-4 py-2.5 text-sm font-semibold text-slate-200 hover:bg-slate-900">Open roadmap <ArrowRight size={15} /></Link></div>
        </header>
