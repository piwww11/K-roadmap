...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    setHydrated(true);
    try {
      const stored = window.localStorage.getItem(CUSTOM_MAJORS_KEY);
      if (stored) setSavedMajors(JSON.parse(stored));
    } catch {
      // Ignore malformed local preference data.
    }
  }, []);

  const rememberMajor = (value: string) => {
    ...
    setSavedMajors((current) => {
      ...
      window.localStorage.setItem(CUSTOM_MAJORS_KEY, JSON.stringify(next));
      return next;
    });
  };
