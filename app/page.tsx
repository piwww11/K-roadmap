...
import { CheckCircle2, Compass, FileText, Target, Wallet, Zap, FlaskConical } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const focusSource = decisionResult?.recommendationStatus !== 'insufficient-evidence' && decisionResult ? 'Latest decision + exploration evidence' : 'Highest current interest';
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-8 font-sans text-slate-100">
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-lg"><div className="mb-3 flex items-center justify-between"><div><h2 className="mb-1 text-sm font-semibold uppercase tracking-wider text-slate-400">Current Focus</h2><div className="text-xl font-bold text-white">{currentFocus ? `${currentFocus.icon} ${currentFocus.name}` : 'Loading...'}</div></div><div className="rounded-xl bg-cyan-500/10 p-3 text-cyan-400"><Target size={24} /></div></div><p className="text-xs text-slate-500">{focusSource}{currentFocusAnalysis ? ` · ${currentFocusAnalysis.score}/100` : ''}</p><p className="mt-3 line-clamp-2 text-sm text-slate-300">{currentFocus?.description ?? 'Your major exploration focus will appear here.'}</p>{decisionResult && <a href="/majors/decision" className="mt-4 inline-block text-xs font-semibold text-indigo-400 hover:text-indigo-300">View decision analysis →</a>}</div>
