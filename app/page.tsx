...
export default function Dashboard() {
  ...
  const getCompletedTaskCount = useJourneyStore(
    (state) => state.getCompletedTaskCount
  );
  const getTotalTaskCount = useJourneyStore(
    (state) => state.getTotalTaskCount
  );
  ...
  const currentPhaseProgress =
    hasHydrated && currentPhase ? getPhaseProgress(currentPhase.id) : 0;
      ...
      <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg">
          ...
          <div className="w-full bg-slate-800 rounded-full h-2.5">
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${currentPhaseProgress}%` }}
              className="bg-indigo-500 h-2.5 rounded-full"
            />
          </div>
          <p className="text-xs text-slate-500 mt-3">
            {currentPhaseProgress}% complete
            {currentPhase ? ` · ${currentPhase.subtitle}` : ''}
          </p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="text-xl font-bold text-white">
                {currentFocus ? `${currentFocus.icon} ${currentFocus.name}` : 'Loading...'}
              </div>
            </div>
          ...
          <p className="text-xs text-slate-500">
            Highest current interest · {currentFocus?.interestScore ?? 0}/10
          </p>
          <p className="text-sm text-slate-300 mt-3 line-clamp-2">
            {currentFocus?.description ?? 'Your major exploration focus will appear here.'}
          </p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-3xl font-bold text-white">{documentProgress}%</div>
              <p className="text-xs text-slate-500 mt-2">
                {completedDocuments.length} of {documents.length} documents ready
              </p>
            </div>
            <div className="p-3 rounded-xl bg-cyan-500/10 text-cyan-400">
              <FileText size={24} />
      ...
      <div className="grid grid-cols-1 gap-6 mb-8 lg:grid-cols-3">
        <div className="lg:col-span-2 bg-gradient-to-br from-indigo-900/40 to-slate-900 border border-indigo-500/30 p-6 rounded-2xl shadow-lg relative overflow-hidden">
          ...
          {todaysMission ? (
            <div>
              ...
              <h3 className="text-2xl font-bold text-white mb-6 max-w-2xl">
                {todaysMission.title}
              </h3>
              <button
                onClick={() => toggleTask(todaysMission.id)}
                className="flex items-center gap-2 rounded-xl bg-indigo-600 px-6 py-3 font-medium text-white shadow-lg shadow-indigo-950/40 transition-all duration-200 hover:-translate-y-0.5 hover:bg-indigo-500 active:translate-y-0"
              >
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-lg">
          ...
          <h3 className="text-xl font-bold text-white mb-3">
            {nextMilestone?.title ?? 'Journey complete'}
          </h3>
          <p className="text-sm text-slate-400 leading-relaxed">
            {nextMilestone?.description ?? 'You have completed every goal in the current phase.'}
          </p>
          {currentPhase && (
      ...
      <div className="grid grid-cols-1 gap-6 mb-8 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-lg">
          ...
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">planned expenses</p>
            </div>
            <Wallet className="text-emerald-400" size={26} />
          </div>
          <div className="mt-5 flex justify-between text-xs text-slate-500">
            <span>Savings: {formatCurrency(hasHydrated ? budget.currentSavings : 0)}</span>
            <span>Target: {formatCurrency(hasHydrated ? budget.targetAmount : 0)}</span>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-lg">
          ...
          <p className="text-sm leading-relaxed text-slate-300">
            Explore → Experiment → Reflect → Compare → Decide → Prepare → Apply
          </p>
          <p className="text-xs text-slate-500 mt-3">
            Your current focus can evolve as your experiences and interests change.
          </p>
        </div>
      </div>
