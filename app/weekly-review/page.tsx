...
export default function WeeklyReviewPage() {
  ...
  const review = useMemo(() => {
    ...
    return buildWeeklyIntelligenceReview({ phases, experiments, majors, analyses: result?.analyses ?? [] });
  ...
  return (
    <main className="min-h-screen bg-slate-950 p-8 text-slate-100">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mb-8 rounded-2xl border border-slate-800 bg-slate-900 p-6"><div className="mb-5 flex items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-widest text-cyan-400">Current interest leader</p><h2 className="mt-1 text-2xl font-bold text-white">{leader ? `${leader.icon} ${leader.name}` : 'No clear leader yet'}</h2></div>{leader && <span className="rounded-full bg-cyan-400/10 px-3 py-1.5 text-xs font-bold text-cyan-300">{leader.adaptiveScore}/100</span>}</div>{leader ? <div className="grid gap-3 md:grid-cols-3"><div className="rounded-xl bg-slate-950/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Adaptive score</p><p className="mt-1 text-xl font-bold text-white">{leader.adaptiveScore}</p></div><div className="rounded-xl bg-slate-950/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Evidence score</p><p className="mt-1 text-xl font-bold text-cyan-300">{leader.evidenceScore}</p></div><div className="rounded-xl bg-slate-950/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">Evidence maturity</p><p className="mt-1 text-xl font-bold text-white">{Math.round(leader.maturity * 100)}%</p></div></div> : <p className="text-sm text-slate-500">Run the Major Decision questionnaire first to establish an initial intelligence baseline.</p>}<Link href="/majors/decision" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-cyan-300 hover:text-cyan-200">Inspect decision evidence <ArrowRight size={15} /></Link></section>
