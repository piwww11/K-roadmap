      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-10">
          ...
          <p className="mt-3 max-w-3xl text-slate-400">
            A deterministic, explainable analysis of your decision answers and real roadmap exploration. This is evidence, not a final verdict.
          </p>
        </header>
        ...
        {decisions.length === 0 ? (
          ...
          <>
            <section className="mb-8 flex flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 p-6 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="text-sm text-slate-500">Decision snapshot</p>
                <h2 className="mt-1 text-2xl font-bold">{new Date(response.timestamp).toLocaleDateString()}</h2>
              </div>
              <select value={selected} onChange={(event) => setSelected(Number(event.target.value))} className="rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white">
                {decisions.map((decision, index) => <option key={`${decision.timestamp}-${index}`} value={index}>Decision #{index + 1} — {new Date(decision.timestamp).toLocaleDateString()}</option>)}
              </select>
            </section>
            ...
            <section className="mb-8 rounded-2xl border border-indigo-500/30 bg-indigo-500/5 p-8">
              ...
              <p className="mt-3 text-slate-300">
                {result.recommendationStatus === 'strong-fit' && 'This major currently has the strongest alignment and enough exploration evidence to be considered a strong fit.'}
                {result.recommendationStatus === 'leading' && 'This major is currently leading, but keep collecting real-world evidence before committing.'}
                {result.recommendationStatus === 'exploring' && 'The signals are close. Treat this as an exploration result rather than a winner.'}
                {result.recommendationStatus === 'insufficient-evidence' && 'There is not enough real exploration evidence to make a meaningful recommendation yet.'}
              </p>
            </section>
                  ...
                  <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                      <div className="flex items-center gap-3">
                        <span className="rounded-full bg-slate-800 px-3 py-1 text-xs text-slate-400">#{index + 1}</span>
                        <h3 className="text-2xl font-bold text-white">{majorName(analysis.majorId)}</h3>
                      </div>
                      <p className="mt-2 text-sm text-slate-500">Evidence: {analysis.evidenceLevel} · Exploration: {analysis.completedExplorationTasks}/{analysis.totalExplorationTasks} tasks</p>
                    </div>
            ...
            <section className="mt-8 rounded-2xl border border-slate-800 bg-slate-900 p-6">
              <h2 className="text-lg font-semibold">Decision evidence used</h2>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {QUESTIONS.map((question) => <div key={question} className="rounded-xl bg-slate-950/60 p-4"><p className="text-xs uppercase tracking-wider text-slate-500">{LABELS[question]}</p><p className="mt-1 text-sm text-slate-300">{response[question]}</p></div>)}
              </div>
            </section>
          </>
        ) : null}
